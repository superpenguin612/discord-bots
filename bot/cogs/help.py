...
class HelpCommand(commands.MinimalHelpCommand):
    ...
    async def send_pages(self, embed: discord.Embed) -> None:
    ...
    async def send_bot_help(self, mapping) -> None:
        ...
        for cog in list(bot.cogs.values()):
            ...
            embed.add_field(
                name=re.sub("_", " ", cog.qualified_name).title(),
                value=f"`{ctx.prefix}help {cog.qualified_name}`\n{joined}",
                inline=False,
            )
    ...
    async def send_cog_help(self, cog: commands.Cog) -> None:
        ...
        embed = tools.create_embed(
            self.context,
            f'Module: {re.sub("_", " ", cog.qualified_name).title()}',
    ...
    async def send_group_help(self, group: commands.Group) -> None:
        ...
        embed = tools.create_embed(
            self.context,
            f"Command Group: {ctx.prefix}{group.qualified_name}",
    ...
    async def send_command_help(self, command: commands.Command) -> None:
        ...
        embed = tools.create_embed(
            self.context,
            f"Command: {ctx.prefix}{command.qualified_name}",
