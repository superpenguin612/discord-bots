...
class Settings(commands.Cog, name='settings'):
    ...
    async def add_record(self, server_id, json):
        return await self.bot.db.execute('INSERT INTO settings (server_id, json) VALUES $1, $2',
            server_id, json)
    
    async def edit_record(self, server_id, json):
        return await self.bot.db.execute('UPDATE settings SET json=$1 WHERE server_id=$2',
            json, server_id)
    ...
    async def editsettings_starboard_channel(self, ctx, channel):
        if type(channel) == discord.channel.TextChannel:
            ...
            embed = tools.create_embed(ctx, 'Edit Settings', desc='The starboard channel has been updated successfully.')
            await ctx.send(embed=embed)
        else:
    ...
    async def editsettings_starboard_numberrequired(self, ctx, number):
        ...
        await self.edit_record(ctx.guild.id, json.dumps(server_settings))
        embed = tools.create_embed(ctx, 'Edit Settings', desc='The number of stars required for the starboard has been updated successfully.')
        await ctx.send(embed=embed)
    ...
    async def editsettings_starboard_reaction(self, ctx, emoji):
        ...
        await self.edit_record(ctx.guild.id, json.dumps(server_settings))
        embed = tools.create_embed(ctx, 'Edit Settings', desc='The reaction to use for the starboard has been updated successfully.')
        await ctx.send(embed=embed)
    ...
    async def editsettings_dailyreport_channel(self, ctx, channel):
        if type(channel) == discord.channel.TextChannel:
            ...
            server_settings['starboard']['channel'] = channel.id
            await self.edit_record(ctx.guild.id, json.dumps(server_settings))
            embed = tools.create_embed(ctx, 'Edit Settings', desc='The daily report channel has been updated successfully.')
            await ctx.send(embed=embed)
        else:
            embed = tools.create_error_embed(ctx, 'The channel must be a text channel.')
            await ctx.send(embed=embed)
    ...
    async def editsettings_suggestions_channel(self, ctx, channel):
        if type(channel) == discord.channel.TextChannel:
            ...
            await self.edit_record(ctx.guild.id, json.dumps(server_settings))
            embed = tools.create_embed(ctx, 'Edit Settings', desc='The suggestions channel has been updated successfully.')
            await ctx.send(embed=embed)
        else:
            embed = tools.create_error_embed(ctx, 'The channel must be a text channel.')
            await ctx.send(embed=embed)
    ...
    async def editsettings_suggestions_upemoji(self, ctx, emoji):
        ...
        await self.edit_record(ctx.guild.id, json.dumps(server_settings))
        embed = tools.create_embed(ctx, 'Edit Settings', desc='The up emoji for suggestions has been updated successfully.')
        await ctx.send(embed=embed)
