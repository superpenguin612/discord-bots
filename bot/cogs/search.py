...
class Search(commands.Cog, name="search"):
    ...
    async def get_mv_list(self, date):
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(search_url) as r:
                js = await r.json()
                food_items.append(
                    js["menuSchedules"][0]["menuBlocks"][0]["cafeteriaLineList"][
                        "data"
                    ][0]["foodItemList"]["data"]
                )
            ...
            search_url = (
                f"https://api.mealviewer.com/api/v4/school/CarmelHigh/{date}/{date}/"
            )
            async with session.get(search_url) as r:
                js = await r.json()
                main_caf = []
                for item in js["menuSchedules"][0]["menuBlocks"][0][
                    "cafeteriaLineList"
                ]["data"]:
    ...
    async def mealviewer_list(self, ctx, date=None):
        date = (
            date.strptime("%m/%d/%Y").strftime("%m-%d-%Y")
            if date
    ...
    @cog_ext.cog_subcommand(
        ...
        options=[
            create_option(
                ...
                required=True,
                choices=[
                    create_choice(name="Freshman Center", value="0"),
                    create_choice(name="Greyhound Station", value="1"),
                    create_choice(name="Main Cafeteria", value="2"),
                ],
            ),
            create_option(
                name="item_number",
    ...
    async def mealviewer_item(self, ctx, cafeteria, item_number, date=None):
        ...
        if food_item["imageFileName"]:
            embed.set_image(
                url=f'https://appassets.mealviewer.com/fooditemimages/{food_item["imageFileName"]}'
            )
