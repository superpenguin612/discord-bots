...
class ReactionRoles(commands.Cog, name="reaction_roles"):
    ...
    async def get_records_by_message_id(self, message_id):
        return await self.bot.db.fetch(
            "SELECT * FROM reaction_roles WHERE message_id=$1;", str(message_id)
        )

    async def get_records_by_server_id(self, server_id):
        return await self.bot.db.fetch(
            "SELECT * FROM reaction_roles WHERE server_id=$1;", str(server_id)
        )
    ...
    async def add_record(self, server_id, channel_id, message_id, role_id, emoji):
        return await self.bot.db.fetchrow(
            "INSERT INTO reaction_roles (server_id, channel_id, message_id, role_id, emoji) VALUES ($1, $2, $3, $4, $5) RETURNING *;",
            server_id,
            channel_id,
            message_id,
            role_id,
    ...
    async def remove_record(self, id):
        return await self.bot.db.fetch(
            "DELETE FROM reaction_roles WHERE id=$1", str(id)
        )
    ...
    async def reactionroles_add(self, ctx: SlashContext):
        def check(msg):
    ...
    async def reactionroles_delete(self, ctx: SlashContext, id: str):
