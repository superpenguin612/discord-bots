...
class Games(commands.Cog, name="games"):
    ...
    async def tictactoe(self, ctx: SlashContext, player2: discord.User) -> None:
        ...
        try:
            ...
            embed = tools.create_error_embed(
                ctx, f"Sorry, {player2.mention} didn't respond in time!"
            )
        ...
        if reaction.emoji == "👍":
            embed = tools.create_embed(
                ctx,
                "Tic Tac Toe Request",
                desc=f"{player2.mention} accepted {ctx.author.mention}'s request to play Tic Tac Toe.",
            )
            await request_msg.edit(embed=embed)
            ...
            embed = tools.create_embed(
                ctx,
                "Tic Tac Toe Request",
                desc=f"{player2.mention} rejected {ctx.author.mention}'s request to play Tic Tac Toe.",
            )
            await request_msg.edit(embed=embed)
            await request_msg.clear_reactions()
            return
    ...
    @commands.command(name="tictactoe", aliases=["tic", "ttt"])
    ...
    def create_game_embed(self, game, board_text):
        ...
        if game["winner"]:
            if game["winner"] == "draw":
                ...
                embed.add_field(
                    name="Winner", value=f'{game[game["winner"]].mention} won!'
                )
        else:
            embed.add_field(
    ...
    def check_victory(self, game):
        ...
        for i in range(0, 2):
            if (
                game["board"][iter_list[i][0]]
                == game["board"][iter_list[i][1]]
                == game["board"][iter_list[i][2]]
            ):
                winner = game["board"][iter_list[i][0]]
        # horizontal
        for i in range(0, 2):
            if (
                game["board"][iter_list[0][i]]
                == game["board"][iter_list[1][i]]
                == game["board"][iter_list[2][i]]
            ):
                winner = game["board"][iter_list[0][i]]

        # diagonal
        if (
            game["board"][iter_list[0][0]]
            == game["board"][iter_list[1][1]]
            == game["board"][iter_list[2][2]]
        ):
            winner = game["board"][iter_list[1][1]]

        # anti-diagonal
        if (
            game["board"][iter_list[2][0]]
            == game["board"][iter_list[1][1]]
            == game["board"][iter_list[0][2]]
        ):
            winner = game["board"][iter_list[1][1]]
    ...
    async def on_reaction_add(self, reaction, user):
        ...
        if active_game and (user.id != self.bot.user.id):
            ...
            if (
                user.id == active_game[active_game["turn"]].id
                and not active_game["winner"]
            ):
                if active_game["board"][location] == "":
                    await self.update_game(
                        reaction.message.id, active_game, location, active_game["turn"]
                    )
