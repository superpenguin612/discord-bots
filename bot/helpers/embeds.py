...
class EmbedCreator():
    def __init__(self, bot: commands.Bot, ctx: SlashContext, channel_id: int, message_id: int):
        self.bot = bot
        self.ctx = ctx
    ...
    def create_menu():
        return discord.Embed(title='Embed Creator | Menu', desc=[
                'Title - :scroll:',
    ...
    async def run(self):
        ...
        self.bot_message = await self.ctx.send(embeds=[self.embed_viewer, self.embed_creator])
        ...
        while running:
            try:
                user_reaction = await self.bot.wait_for("reaction", check=rcheck, timeout=180)
            except asyncio.TimeoutError:
                await self.bot_message.clear_reactions()
            
            self.setup_status = self.REACTIONS_CONVERTER[self.setup_status][user_reaction]
            ...
            if self.setup_status == 'menu':
                ...
                if self.setup_status == 'title':
                    self.embed_creator = discord.Embed(title='Embed Creator | Title', description='Send the title you want for the embed.')
                    try:
                        title = await self.bot.wait_for("message", check=tcheck, timeout=180)
                    except asyncio.TimeoutError:
                        return
                    self.embed_viewer.title = title
                if self.setup_status == 'title':
                    self.embed_creator = discord.Embed(title='Embed Creator | Description', description='Send the title you want for the embed.')
                    try:
                        title = await self.bot.wait_for("message", check=tcheck, timeout=180)
                    except asyncio.TimeoutError:
                        return
                    self.embed_viewer.title = title
                if self.setup_status == 'title':
                    self.embed_creator = discord.Embed(title='Embed Creator | Footer', description='Send the title you want for the embed.')
                    try:
                        title = await self.bot.wait_for("message", check=tcheck, timeout=180)
                    except asyncio.TimeoutError:
                        return
                    self.embed_viewer.title = title
                if self.setup_status == 'title':
                    self.embed_creator = discord.Embed(title='Embed Creator | Color', description='Send the title you want for the embed.')
                    try:
                        title = await self.bot.wait_for("message", check=tcheck, timeout=180)
                    except asyncio.TimeoutError:
                        return
                    self.embed_viewer.title = title
                if self.setup_status == 'title':
                    self.embed_creator = discord.Embed(title='Embed Creator | Image', description='Send the title you want for the embed.')
                    try:
                        title = await self.bot.wait_for("message", check=tcheck, timeout=180)
                    except asyncio.TimeoutError:
                        return
                    self.embed_viewer.title = title
                if self.setup_status == 'title':
                    self.embed_creator = discord.Embed(title='Embed Creator | Thumbnail', description='Send the title you want for the embed.')
                    try:
                        title = await self.bot.wait_for("message", check=tcheck, timeout=180)
                    except asyncio.TimeoutError:
                        return
                    self.embed_viewer.title = title
                if self.setup_status == 'title':
                    self.embed_creator = discord.Embed(title='Embed Creator | Author', description='Send the title you want for the embed.')
                    try:
                        title = await self.bot.wait_for("message", check=tcheck, timeout=180)
                    except asyncio.TimeoutError:
                        return
                    self.embed_viewer.title = title
                if self.setup_status == 'title':
                    self.embed_creator = discord.Embed(title='Embed Creator | Author Icon', description='Send the title you want for the embed.')
                    try:
                        title = await self.bot.wait_for("message", check=tcheck, timeout=180)
                    except asyncio.TimeoutError:
                        return
                    self.embed_viewer.title = title
                if self.setup_status == 'title':
                    self.embed_creator = discord.Embed(title='Embed Creator | Fields', description='Send the title you want for the embed.')
                    try:
                        title = await self.bot.wait_for("message", check=tcheck, timeout=180)
                    except asyncio.TimeoutError:
                        return
